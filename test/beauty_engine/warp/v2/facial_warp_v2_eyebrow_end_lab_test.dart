...
import 'package:editaiapp/features/editor/beauty_engine/warp/v2/eyebrow_end/eyebrow_end_masks.dart';
...
const _dumpRoot = '.cursor/facial-warp-v2/eyebrow_end/B';
...
void main() {
  ...
  test(
    'Eyebrow End B lab matrix p01/p05/p12 × geral ±100/±50/0 + L/R writes v2Raw without fill',
    () {
      ...
      for (final photo in photos) {
        for (final level in _levels) {
          ...
          expect(nonzeroDy, 0, reason: '${photo.id} ${level.label} Δx only');
          ...
          if (isIdentity) {
            ...
            expect(
              ...
              reason: '${photo.id} ${level.label} must stay subtle',
            );
            expect(
            ...
            if (level.tPhotoLeft == null) {
              if (level.t < 0) {
                expect(m.browsJoin, isTrue, reason: photo.id);
                expect(m.dxAtInnerLeft, lessThan(0), reason: photo.id);
                expect(m.dxAtInnerRight, greaterThan(0), reason: photo.id);
              } else if (level.t > 0) {
                expect(m.browsSeparate, isTrue, reason: photo.id);
                expect(m.dxAtInnerLeft, greaterThan(0), reason: photo.id);
                expect(m.dxAtInnerRight, lessThan(0), reason: photo.id);
              }
              expect(
                m.dxAtTailLeft.abs(),
                lessThan(0.25 * m.dxAtInnerLeft.abs() + 1e-6),
                reason: '${photo.id} tail 300',
              );
              expect(
                m.dxAtTailRight.abs(),
                lessThan(0.25 * m.dxAtInnerRight.abs() + 1e-6),
                reason: '${photo.id} tail 70',
              );
              expect(
                m.dxAtArchLeft.abs(),
                lessThan(0.45 * m.dxAtInnerLeft.abs() + 1e-6),
                reason: '${photo.id} arch 334',
              );
              expect(
                m.dxAtArchRight.abs(),
                lessThan(0.45 * m.dxAtInnerRight.abs() + 1e-6),
                reason: '${photo.id} arch 105',
              );
            } else if (level.tPhotoLeft! > 0 && level.tPhotoRight == 0) {
              expect(
                m.dxAtInnerRight.abs(),
                greaterThan(m.dxAtInnerLeft.abs() + 0.15),
                reason: '${photo.id} L100 should move 107',
              );
              expect(m.dxAtInnerRight, lessThan(0), reason: photo.id);
            } else if (level.tPhotoRight! > 0 && level.tPhotoLeft == 0) {
              expect(
                m.dxAtInnerLeft.abs(),
                greaterThan(m.dxAtInnerRight.abs() + 0.15),
                reason: '${photo.id} R100 should move 336',
              );
              expect(m.dxAtInnerLeft, greaterThan(0), reason: photo.id);
            }
          }
  ...
  test('Eyebrow End B lab test does not import V1 fill, pipeline or controller',
      () {
    final imports = File(
    ...
    expect(imports.contains('eyebrow_height_field.dart'), isFalse);
    expect(imports.contains('eyebrow_width_field.dart'), isFalse);
  });
}
