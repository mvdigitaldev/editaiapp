...
void main() {
  ...
  test('V2.2 lab matrix p01/p05/p12 × jaw 0/25/50 writes v2Raw without fill', () {
    ...
    for (final photo in photos) {
      for (final level in _levels) {
        ...
        if (level.t == 0) {
          ...
          expect(built.metrics.gonionNarrows, isTrue, reason: photo.id);
          expect(built.metrics.minDetJ, greaterThan(0), reason: photo.id);
          expect(built.metrics.outsideJawZoneP95, lessThanOrEqualTo(_protectEps));
          ...
          expect(built.metrics.beard.p95Abs, lessThanOrEqualTo(_protectEps));
          expect(built.metrics.ears.p95Abs, lessThanOrEqualTo(_protectEps));
        }

        final dir = Directory(
          '.cursor/facial-warp-v2/v2.2/${photo.dumpId}/${level.label}',
        );
    ...
    File('.cursor/facial-warp-v2/v2.2/summary.json').writeAsStringSync(
    ...
    for (final photo in photos) {
      for (final level in _levels) {
        expect(
          File(
            '.cursor/facial-warp-v2/v2.2/${photo.dumpId}/${level.label}/v2Raw.png',
          ).existsSync(),
          isTrue,
        );
      }
  ...
  test('V2.2 lab test does not import V1 fill, pipeline or controller', () {
    final imports = File(
