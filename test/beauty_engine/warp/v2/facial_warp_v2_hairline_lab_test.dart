...
const _dumpRoot = '.cursor/facial-warp-v2/hairline/B';
...
void main() {
  ...
  test(
    'Hairline B lab matrix p01/p05/p12 × t -50/-25/0/25/50 writes v2Raw without fill',
    () {
      ...
      for (final photo in photos) {
        for (final level in _levels) {
          ...
          if (level.t == 0) {
            ...
            expect(invalidCount, 0, reason: '${photo.id} t=${level.t} fill');
            expect(built.metrics.minDetJ, greaterThan(0), reason: photo.id);
            expect(
              built.metrics.outsideHairlineZoneP95,
            ...
            expect(built.metrics.cheeks.p95Abs, lessThanOrEqualTo(_protectEps));
            expect(built.metrics.jawDomain.p95Abs, lessThanOrEqualTo(_protectEps));
            final peak = built.metrics.dyAtCrown.abs();
            expect(peak, greaterThan(0), reason: photo.id);
            expect(
              built.metrics.absAtTempleMax,
              lessThan(0.35 * peak),
              reason: '${photo.id} temple tail vs crown',
            );
            if (level.t < 0) {
              expect(built.metrics.hairlineInflates, isTrue, reason: photo.id);
              expect(built.metrics.dyAtCrown, lessThan(0), reason: photo.id);
            } else {
              expect(built.metrics.hairlineDeflates, isTrue, reason: photo.id);
              expect(built.metrics.dyAtCrown, greaterThan(0), reason: photo.id);
            }
          }
  ...
  test('Hairline B lab test does not import V1 fill, pipeline or controller', () {
    final imports = File(
      'test/beauty_engine/warp/v2/facial_warp_v2_hairline_lab_test.dart',
    )
        .readAsLinesSync()
...
void _saveOwnership(
  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      ...
      final g = masks.hairlineActive[i] != 0 ? 200 : 20;
