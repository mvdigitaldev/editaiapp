...
/// p05 primeiro (sign-off), depois p12, depois p01 (vazamento maxilar).
const _assets = {
...
const _dumpRoot = '.cursor/facial-warp-v2/cheekbones/H';
...
void main() {
  ...
  setUpAll(() {
    ...
    expect(photos.map((p) => p.dumpId).toList(), ['p05', 'p12', 'p01']);
  });

  test(
    'Cheekbones B lab matrix p05/p12/p01 × cheekbone 0/25/50 writes v2Raw without fill',
    () {
      ...
      for (final row in summary) {
        ...
        expect(row['cheekbonesNarrows'], isTrue, reason: '${row['id']} t=$t');
        expect(
          (row['minDetJ'] as num).toDouble(),
          greaterThan(0),
          reason: '${row['id']} t=$t detJ=${row['minDetJ']}',
        );
        expect(
          (row['outsideCheekZoneP95'] as num).toDouble(),
          lessThanOrEqualTo(_protectEps),
        );
        expect((row['absAtChinTip'] as num).toDouble(), lessThanOrEqualTo(_protectEps));
        final influenceMax = (row['influenceMax'] as num).toDouble();
        expect(
          (row['absAtGonionLeft'] as num).toDouble(),
          lessThan(0.55 * influenceMax),
          reason: '${row['id']} t=$t gonion tail stronger than malar',
        );
        expect(
          (row['absAtGonionRight'] as num).toDouble(),
          lessThan(0.55 * influenceMax),
          reason: '${row['id']} t=$t gonion tail stronger than malar',
        );
      }
  ...
  test('Cheekbones B lab test does not import V1 fill, pipeline, Fields or controller',
      () {
    final imports = File(
    ...
    expect(imports.contains('face_slim_field.dart'), isFalse);
  });
}
...
void _writeArtifacts({
  ...
  _saveMask('$dir/cheekActive.png', masks.cheekActive, photo.width, photo.height);
  _saveOwnership(
...
void _saveOwnership(
  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      ...
      final g = masks.cheekActive[i] != 0 ? 200 : 20;
