...
import 'package:editaiapp/features/editor/beauty_engine/warp/v2/eyebrow_width/eyebrow_width_masks.dart';
...
const _dumpRoot = '.cursor/facial-warp-v2/eyebrow_width/B';
...
void main() {
  ...
  test(
    'Eyebrow Width B lab matrix p01/p05/p12 × geral ±100/±50/0 + L/R writes v2Raw without fill',
    () {
        ...
        for (final level in _levels) {
          ...
          if (isIdentity) {
            ...
            expect(
              ...
              reason: '${photo.id} ${level.label} must stay a light thicken',
            );
            expect(
            ...
            final peak = math.max(
              math.max(m.dyAtPrimaryLeft.abs(), m.dyAtPrimaryRight.abs()),
              math.max(m.dyAtLowerLeft.abs(), m.dyAtLowerRight.abs()),
            );
            expect(
              m.absAtLidLeft,
            ...
            if (level.tPhotoLeft == null) {
              if (level.t < 0) {
                expect(m.browsThin, isTrue, reason: photo.id);
              } else if (level.t > 0) {
                expect(m.browsThicken, isTrue, reason: photo.id);
              }
              ...
              expect(m.dyAtLowerRight, greaterThan(0), reason: photo.id);
              ...
              expect(m.dyAtLowerLeft, greaterThan(0), reason: photo.id);
            }
          }
  ...
  test(
      'Eyebrow Width B lab test does not import V1 fill, pipeline or controller',
      () {
    final imports = File(
