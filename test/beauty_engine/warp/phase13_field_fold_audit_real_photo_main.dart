...
import 'package:editaiapp/features/editor/beauty_engine/warp/face_warp_field_fold_audit_diagnostic.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as img;
import 'package:path_provider/path_provider.dart';

/// Harness Fase 13 — auditoria field fold p26 (pexels-1130626).
///
/// flutter run -t test/beauty_engine/warp/phase13_field_fold_audit_real_photo_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    const photoAsset =
        'test/beauty_engine/warp/fixtures/phase12/p26.jpg';
    ...
    try {
      ...
      debugPrint('PHASE13_FIELD_FOLD_OUTPUT_DIR=${docsDir.path}');

      final result = await FaceWarpFieldFoldAuditDiagnostic.run(
        face: face,
        mesh: mesh,
        width: source.width,
        ...
        backgroundRgba: rgba,
        photoId: 'p26',
        runId: 'field-fold-audit-p26',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('field_fold_audit_failed');
      }

      debugPrint('PHASE13_FIELD_FOLD_OK');
      debugPrint('recommendation=${result.summary['conclusion']?['integrationRecommendation']}');
      debugPrint('summary=${result.summaryJsonPath}');
    } catch (e, st) {
      debugPrint('PHASE13_FIELD_FOLD_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(
          child: Text('Phase 13 field fold audit running…'),
        ),
      ),
    );
  }
