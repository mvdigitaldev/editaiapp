...
import 'package:editaiapp/features/editor/beauty_engine/warp/face_warp_injectivity_diagnostic.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as img;
import 'package:path_provider/path_provider.dart';

/// Harness Fase 6 — intensity sweep injetividade @ foto real.
///
/// flutter run -t test/beauty_engine/warp/injectivity_sweep_real_photo_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      debugPrint('INJECTIVITY_SWEEP_OUTPUT_DIR=${docsDir.path}');

      final result = await FaceWarpInjectivityDiagnostic.run(
        face: face,
        mesh: mesh,
        ...
        runId: 'injectivity-sweep-man-5021469',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('injectivity_diagnostic_failed');
      }

      debugPrint('INJECTIVITY_SWEEP_OK');
      debugPrint('firstMultiCoverage=${result.summary['firstMultiCoverageIntensity']}');
      debugPrint('firstOverlap=${result.summary['firstSignificantOverlapIntensity']}');
      debugPrint('firstJump10=${result.summary['firstJump10Intensity']}');
      debugPrint('json=${result.sweepJsonPath}');
    } catch (e, st) {
      debugPrint('INJECTIVITY_SWEEP_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(child: Text('Injectivity sweep running…')),
      ),
    );
  }
}
