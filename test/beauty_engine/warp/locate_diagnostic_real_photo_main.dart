...
/// Harness Fase 3 — diagnóstico locate + métricas pós-correção.
///
/// flutter run -t test/beauty_engine/warp/locate_diagnostic_real_photo_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      debugPrint('LOCATE_DIAG_OUTPUT_DIR=${docsDir.path}');

      final result = await TriMeshLocateDiagnostic.run(
        sourceRgba: source.bytes,
        width: source.width,
        height: source.height,
        payload: payload,
        runId: 'locate-real-man-5021469-90',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('locate_diagnostic_failed');
      }

      debugPrint('LOCATE_DIAG_OK');
      debugPrint('fullScan=${result.fullScanComparison}');
      debugPrint('pureRemap=${result.pureRemapMetrics}');
      debugPrint('epsilon=${result.epsilonSweep}');
      debugPrint('neighbor=${result.neighborDiscontinuities}');
      debugPrint('jsonl=${result.candidatesJsonl}');
      debugPrint('bugPng=${result.locateBugPng}');
      debugPrint('discPng=${result.locateDiscontinuitiesPng}');
    } catch (e, st) {
      debugPrint('LOCATE_DIAG_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(child: Text('Locate diagnostic running…')),
      ),
    );
  }
}
