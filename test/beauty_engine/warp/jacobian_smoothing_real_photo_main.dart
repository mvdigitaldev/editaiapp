...
import 'package:editaiapp/features/editor/beauty_engine/warp/face_warp_jacobian_smoothing_diagnostic.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as img;
import 'package:path_provider/path_provider.dart';

/// Harness Fase 10 — smoothing de escalas pós-Phase9.
///
/// flutter run -t test/beauty_engine/warp/jacobian_smoothing_real_photo_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      debugPrint('JACOBIAN_SMOOTHING_OUTPUT_DIR=${docsDir.path}');

      final result = await FaceWarpJacobianSmoothingDiagnostic.run(
        face: face,
        mesh: mesh,
        width: source.width,
        height: source.height,
        personMask: personMask,
        runId: 'jacobian-smoothing-man-5021469',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('jacobian_smoothing_diagnostic_failed');
      }

      debugPrint('JACOBIAN_SMOOTHING_OK');
      debugPrint('baseline=${result.summary['baselinePreserved']}');
      debugPrint('allSafety=${result.summary['allSafetyPassed']}');
      debugPrint('bestAlpha=${result.summary['bestAlpha']}');
      debugPrint('summary=${result.summaryJsonPath}');
    } catch (e, st) {
      debugPrint('JACOBIAN_SMOOTHING_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(child: Text('Jacobian smoothing diagnostic running…')),
      ),
    );
  }
}
