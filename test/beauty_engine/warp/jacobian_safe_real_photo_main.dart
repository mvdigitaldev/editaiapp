...
import 'package:editaiapp/features/editor/beauty_engine/warp/face_warp_jacobian_safe_diagnostic.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as img;
import 'package:path_provider/path_provider.dart';

/// Harness Fase 8 — Jacobian-safe constraint prototype.
///
/// flutter run -t test/beauty_engine/warp/jacobian_safe_real_photo_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      debugPrint('JACOBIAN_SAFE_OUTPUT_DIR=${docsDir.path}');

      final result = await FaceWarpJacobianSafeDiagnostic.run(
        face: face,
        mesh: mesh,
        width: source.width,
        height: source.height,
        personMask: personMask,
        runId: 'jacobian-safe-man-5021469',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('jacobian_safe_diagnostic_failed');
      }

      debugPrint('JACOBIAN_SAFE_OK');
      ...
      for (final row in result.summary['epsilonComparison'] as List) {
        debugPrint(
          'eps=${row['epsilon']} maxNoFold=${row['maxFaceSlimWithoutFold']} '
          'ret90=${row['retention_at90']}',
        );
      }
    } catch (e, st) {
      debugPrint('JACOBIAN_SAFE_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(child: Text('Jacobian-safe diagnostic running…')),
      ),
    );
  }
}
