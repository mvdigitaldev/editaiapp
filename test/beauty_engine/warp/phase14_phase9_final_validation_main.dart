...
import 'package:editaiapp/features/editor/beauty_engine/warp/face_warp_phase9_final_validation_diagnostic.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as img;
import 'package:path_provider/path_provider.dart';

/// Harness Fase 14 — validação final Phase 9 + Safety Gate (30 fotos).
///
/// flutter run -t test/beauty_engine/warp/phase14_phase9_final_validation_main.dart -d <ios>
...
class _HarnessAppState extends State<_HarnessApp> {
  ...
  Future<void> _run() async {
    try {
      ...
      for (final entry in photos) {
        ...
        debugPrint('P14 loading $id $asset');
        ...
        try {
          ...
          if (face == null) {
            debugPrint('P14 skip $id no_face');
            skipped++;
            continue;
          }
          ...
          debugPrint('P14 skip $id $e');
          skipped++;
        }
      }

      debugPrint('P14 loaded=${inputs.length} skipped=$skipped');

      final docsDir = await getApplicationDocumentsDirectory();
      debugPrint('PHASE14_FINAL_OUTPUT_DIR=${docsDir.path}');

      final result = await FaceWarpPhase9FinalValidationDiagnostic.runBatch(
        photos: inputs,
        runId: 'phase9-final-30-photos',
        outputDirectory: docsDir.path,
      );

      if (result == null) {
        throw StateError('phase9_final_validation_failed');
      }

      debugPrint('PHASE14_FINAL_OK');
      debugPrint('photos=${result.summary['photoCount']}');
      debugPrint('structuralPasses=${result.summary['totals']?['structuralPasses']}');
      debugPrint('structuralFailures=${result.summary['totals']?['structuralFailures']}');
      debugPrint('recommendation=${result.summary['recommendation']}');
      debugPrint('summary=${result.summaryJsonPath}');
    } catch (e, st) {
      debugPrint('PHASE14_FINAL_FAIL $e');
      debugPrint('$st');
    }
  }
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(
          child: Text('Phase 14 final validation running…'),
        ),
      ),
    );
  }
