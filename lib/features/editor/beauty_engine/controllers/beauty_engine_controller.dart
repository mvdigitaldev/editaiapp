...
class BeautyEngineController {
  ...
  final EyebrowWidthFieldRuntime _eyebrowWidthRuntime =
  ...
  /// EyebrowWidthField + remap bilinear. Independente da Altura e do makeup
  /// `eyebrows`. t=0 não chama o renderer.
  Uint8List applyEyebrowWidthWarp({
    required Uint8List sourceRgba,
    required int width,
    required int height,
    required FaceMeshResult? face,
    ...
    final t = (parameters['eyebrow_width'] ?? 0).clamp(-1.0, 1.0);
    final tPhotoLeft = (parameters['eyebrow_width_left'] ?? t).clamp(-1.0, 1.0);
    final tPhotoRight =
        (parameters['eyebrow_width_right'] ?? t).clamp(-1.0, 1.0);
    if (face == null ||
        (tPhotoLeft.abs() <= 1e-6 && tPhotoRight.abs() <= 1e-6) ||
        sourceRgba.length != width * height * 4) {
      return sourceRgba;
    ...
    lastFaceWarpBackend = 'v2_eyebrow_width';
    lastFaceWarpField = null;
    return warped.rgba;
  }

  /// EyebrowEndField + remap bilinear. Independente da Altura/Largura e do
  /// makeup `eyebrows`. t=0 não chama o renderer.
  Uint8List applyEyebrowEndWarp({
    required Uint8List sourceRgba,
    required int width,
    required int height,
    required FaceMeshResult? face,
    ...
    final t = (parameters['eyebrow_end'] ?? 0).clamp(-1.0, 1.0);
    final tPhotoLeft = (parameters['eyebrow_end_left'] ?? t).clamp(-1.0, 1.0);
    final tPhotoRight = (parameters['eyebrow_end_right'] ?? t).clamp(-1.0, 1.0);
    if (face == null ||
        (tPhotoLeft.abs() <= 1e-6 && tPhotoRight.abs() <= 1e-6) ||
        sourceRgba.length != width * height * 4) {
      return sourceRgba;
    ...
    lastFaceWarpBackend = 'v2_eyebrow_end';
    lastFaceWarpField = null;
    return warped.rgba;
  }
  ...
  DisplacementField? _buildFaceWarpStage({
    ...
    switch (faceWarpChainStages[stage].backend) {
      ...
      case 'v2_eyebrow_width':
        return EyebrowWidthField.build(
          face: face,
          imageSize: imageSize,
          t: general,
          tPhotoLeft: left,
