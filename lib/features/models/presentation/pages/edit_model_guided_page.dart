  ...
  Future<void> _analyzeImage() async {
    ...
    if (modeloId == null || modeloId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Modelo inválido. Volte e tente novamente.')),
      );
      return;
    }
    ...
    try {
      ...
      final strings =
          list.map((e) => '$e'.trim()).where((s) => s.isNotEmpty).toList();
      setState(() {
        _analyzing = false;
      ...
      if (code == 401) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                'Erro ao analisar a imagem. Verifique a conexão e tente de novo.'),
          ),
        );
      }
    } catch (_) {
  ...
  Future<void> _handleGenerate() async {
    ...
    if (_selectedSuggestions.isEmpty && notes.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Selecione sugestões ou descreva o que deseja alterar.'),
        ),
      );
      return;
    }
      ...
      if (e is DioException) {
        ...
        if (statusCode == 402) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content:
                    Text('Modelo não encontrado. Volte e tente novamente.')),
          );
                ...
                child: Row(
                  children: [
                    IconButton(
                      ...
                      onPressed: (_isLoading || _analyzing)
                          ? null
                          ...
                          children: [
                            ...
                            AppButton(
                              text: _analyzing
                                  ...
                                  : 'Analisar imagem',
                              onPressed:
                                  (_selectedImagePath == null || _analyzing)
                                      ? null
                                      : _analyzeImage,
                                ...
                                builder: (context, constraints) {
                                  ...
                                  return Wrap(
                                    ...
                                    children: _suggestions.map((s) {
                                      final selected =
                                          _selectedSuggestions.contains(s);
                                      return SizedBox(
                                        width: w,
                                        child: FilterChip(
