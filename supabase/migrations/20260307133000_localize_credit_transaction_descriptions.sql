-- Localizacao de descricoes de transacoes de credito para portugues.

CREATE OR REPLACE FUNCTION public.consume_reserved_credits(
  p_reservation_id uuid,
  p_reference_id uuid,
  p_description text DEFAULT 'Uso em edicao'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    ...
    -v_res.credits,
    CASE
      WHEN lower(COALESCE(btrim(p_description), '')) = 'usage' THEN 'Uso em edicao'
      ELSE COALESCE(NULLIF(btrim(p_description), ''), 'Uso em edicao')
    END,
    p_reference_id
  )
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      'Creditos expirados nao utilizados',
      v_row.id
    );
...
-- Backfill de descricoes existentes em ingles.
UPDATE public.credit_transactions
SET description = 'Uso em edicao'
WHERE type = 'usage'
  AND lower(COALESCE(description, '')) = 'usage';

UPDATE public.credit_transactions
SET description = 'Creditos expirados nao utilizados'
WHERE type = 'credit_expiration'
  AND lower(COALESCE(description, '')) LIKE 'expired unused credits%';
